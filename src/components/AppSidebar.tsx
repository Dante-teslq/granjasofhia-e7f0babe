...
const AppSidebar = ({ onNavigate }: AppSidebarProps) => {
  ...
  // Show the install button only when the app is not yet installed as standalone
  const showInstallButton = !isInstalled && platform !== null;
  ...
  return (
    <>
        ...
        <div className="p-4 border-t border-sidebar-border space-y-3">
          ...
          {showInstallButton && (
            <Button
              ...
              className="w-full justify-start gap-2 text-sidebar-foreground/60 hover:text-sidebar-primary"
            >
      ...
      {showInstallBanner && (
        <PWAInstallBanner forceOpen onClose={() => setShowInstallBanner(false)} />
      )}
    </>
  );
};
